...
def suppressFreqDFTmodel(x, fs, N):
    ...
    #f(k) = fs*k/N: frequency in radian per second
    #70/((fs/2)/511
    k = 70
    #fk = int(np.ceil(k/((fs/2)/(float(N)/2 -1))))
    #    print fk
    #pXfilter[fk] = -120
    ...
    fk = np.ceil(k*fs/float(N))
    for i in range(M):
        if((i-1)*fs/N <= k):
            mXfilter[i] = -120.0
    
    yFilter = dftSynth(mXfilter, pX, w.size)*outputScaleFactor
    
        
    return y, yFilter#, mX, mXfilter, fk
